...
class Attention(torch.nn.Module):
    ...
    def __init__(
        ...
        # public TeleFuser dispatch layer below.
...
class TransformerArgsPreprocessor:
    def __init__(  # noqa: PLR0913
        ...
        keyframes_embedding_provider: Callable[[], torch.Tensor | None] | None = None,
    ) -> None:
    ...
    def prepare(
        ...
        if self.keyframes_embedding_provider is not None and modality.keyframes_mask is not None:
            keyframes_embedding = self.keyframes_embedding_provider()
            if keyframes_embedding is not None:
                x = x + (modality.keyframes_mask > 0).to(dtype=x.dtype) * keyframes_embedding.to(dtype=x.dtype)
        batch_size = x.shape[0]
...
class MultiModalTransformerArgsPreprocessor:
    ...
    def _prepare_cross_attention_timestep(
        ...
        """Prepare cross-attention AdaLN inputs with upstream token granularity."""
...
class LTXModel(torch.nn.Module):
        ...
        *,
        ...
        if model_type.is_video_enabled():
            ...
            if use_keyframes_abs_pos_embedding:
                self.keyframes_abs_pos_embedding = torch.nn.Parameter(torch.zeros(1, self.inner_dim))
            self._init_video(
    ...
    def _keyframes_embedding(self) -> torch.Tensor | None:
        """Resolve the checkpoint-backed keyframe marker on every forward setup."""
...
def build_ltx_model_from_config(config: dict) -> LTXModel:
    ...
    return LTXModel(
        ...
        audio_ff_bias=transformer_config.get("audio_ff_bias", True),
...
class LTXVideoTransformer(BaseModel):
    """Deprecated compatibility wrapper excluded from the LTX-2.5 public surface."""

    def __init__(self) -> None:
        super().__init__()
        raise RuntimeError("LTXVideoTransformer is not available in the isolated LTX-2.5 implementation")
...
_LTX25_TRANSFORMER_PREFIX = "model.diffusion_model."
_LTX25_EMBEDDING_PREFIXES = (
...
def build_ltx25_av_model(config: dict[str, Any]) -> LTXModel:
    """Build the LTX-2.5 AV denoiser strictly from checkpoint metadata."""
    transformer_config = config.get("transformer")
    ...
    if mismatches:
        raise ValueError(f"Unsupported LTX-2.5 AV transformer config: {mismatches}")
    return build_ltx_model_from_config(config)


def ltx25_transformer_key_to_model_key(key: str) -> str | None:
    """Map a split LTX-2.5 checkpoint key into the isolated AV denoiser state dict."""
    if not key.startswith(_LTX25_TRANSFORMER_PREFIX):
        return None
    suffix = key.removeprefix(_LTX25_TRANSFORMER_PREFIX)
    if suffix.startswith(_LTX25_EMBEDDING_PREFIXES):
        return None
    return f"velocity_model.{suffix}"


def ltx25_transformer_checkpoint_key_coverage(
    checkpoint_path: str | Path,
    model_keys: set[str],
) -> tuple[set[str], set[str]]:
    """Return unexplained split-checkpoint keys and missing isolated-model keys."""
    with safe_open(str(checkpoint_path), framework="pt", device="cpu") as checkpoint:
        mapped = {
            target for key in checkpoint.keys() if (target := ltx25_transformer_key_to_model_key(key)) is not None
        }
    return mapped - model_keys, model_keys - mapped


class LTX25AVTransformer(BaseModel):
    """Isolated LTX-2.5 48-layer gated audio-video x0 denoiser."""

    def __init__(self, config: dict[str, Any]) -> None:
        super().__init__()
        self.velocity_model = build_ltx25_av_model(config)
        self.layer_name_list = ["velocity_model.transformer_blocks"]
    ...
    def from_checkpoint(
        ...
        *,
        ...
        """Construct and strictly load the split LTX-2.5 denoiser checkpoint."""
        from .checkpoint import inspect_checkpoint

        metadata = inspect_checkpoint(checkpoint_path)
        with torch.device("meta"):
            model = cls(metadata.config)
        unexpected, missing = ltx25_transformer_checkpoint_key_coverage(checkpoint_path, set(model.state_dict()))
        if unexpected or missing:
            raise ValueError(
                "LTX-2.5 transformer checkpoint coverage mismatch: "
                f"unexpected={sorted(unexpected)[:5]}, missing={sorted(missing)[:5]}"
            )
        with safe_open(str(checkpoint_path), framework="pt", device="cpu") as checkpoint:
            state_dict = {
                target: checkpoint.get_tensor(key)
                for key in checkpoint.keys()
                if (target := ltx25_transformer_key_to_model_key(key)) is not None
            }
        missing_keys, unexpected_keys = model.load_state_dict(state_dict, strict=True, assign=True)
        if missing_keys or unexpected_keys:
            raise ValueError(
                f"LTX-2.5 transformer load mismatch: missing={missing_keys[:5]}, unexpected={unexpected_keys[:5]}"
            )
        # On the supported H100 runtime, upstream selects FlashAttention 4.
        # Keep this fidelity baseline explicit; the public attention dispatch
        # retains its normal fallback behavior when FA4 is unavailable.
        model.set_attention_config(AttentionConfig.dense_attention(AttnImplType.FLASH_ATTN_4))
    ...
    def forward(
        ...
        """Return x0 predictions for the supplied LTX-2.5 modality states."""
        velocity_video, velocity_audio = self.velocity_model(video, audio, perturbations)
        denoised_video = (
            to_denoised(video.latent, velocity_video, video.timesteps) if velocity_video is not None else None
        )
        denoised_audio = (
            to_denoised(audio.latent, velocity_audio, audio.timesteps) if velocity_audio is not None else None
        )
        return denoised_video, denoised_audio
...
__all__ = [
    ...
    "ltx25_transformer_checkpoint_key_coverage",
