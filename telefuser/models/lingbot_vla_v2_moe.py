"""Native fused-MoE action expert used by LingBot-VLA v2.
...
def fused_moe_forward(
    ...
    for expert_id in range(num_experts):
        ...
        gate = torch.nn.functional.linear(expert_input, fc1_1_weight[expert_id])
        up = torch.nn.functional.linear(expert_input, fc1_2_weight[expert_id])
        intermediate = torch.nn.functional.silu(gate) * up
        expert_output = torch.nn.functional.linear(intermediate, fc2_weight[expert_id])
