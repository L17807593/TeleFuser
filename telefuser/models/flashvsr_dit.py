...
class SelfAttention(nn.Module):
    ...
    def async_usp_forward(
        ...
        v_wait = ulysses_scatter_heads(v_4d, sp_group, tag="v", barrier=False)
        q = self.norm_q(self.q(x))
        q = rope_apply(q, freqs, self.num_heads)
        q_4d = rearrange(q, "b s (h d) -> b s h d", h=self.num_heads)
        q_wait = ulysses_scatter_heads(q_4d, sp_group, tag="q", barrier=False)
        k = self.norm_k(self.k(x))
        k = rope_apply(k, freqs, self.num_heads)
        k_4d = rearrange(k, "b s (h d) -> b s h d", h=self.num_heads)
        k_wait = ulysses_scatter_heads(k_4d, sp_group, tag="k")
        q_4d = q_wait()
        k_4d = k_wait()
        v_4d = v_wait()
