...
class FP32LayerNorm(nn.LayerNorm):
    """LayerNorm with upstream-compatible FP32 accumulation."""
...
def get_1d_rotary_pos_embed(
    ...
    *,
    ...
    """Local copy of the Diffusers real-valued 1D RoPE construction."""
    if dim % 2:
    ...
    if not use_real:
        raise ValueError("SwiftVR requires real-valued rotary embeddings")
    if repeat_interleave_real:
        return (
            freqs.cos().repeat_interleave(2, dim=1, output_size=dim).float(),
            freqs.sin().repeat_interleave(2, dim=1, output_size=dim).float(),
        )
...
class PixArtAlphaTextProjection(nn.Module):
    def __init__(self, in_features: int, hidden_size: int, act_fn: str = "gelu_tanh") -> None:
        super().__init__()
        if act_fn != "gelu_tanh":
            raise ValueError(f"Unsupported SwiftVR text projection activation: {act_fn}")
...
class WanAttnProcessor:
    ...
    def __call__(
        ...
        if encoder_hidden_states_img is not None:
            ...
            hidden_states_img = _dense_attn(query, key_img, value_img, attn.attention_config)
            hidden_states_img = hidden_states_img.flatten(2, 3).type_as(query)

        hidden_states = _dense_attn(query, key, value, attn.attention_config, attention_mask)
...
class WanAttention(torch.nn.Module):
    ...
    def __init__(
        ...
        is_cross_attention: bool | None = None,
    ) -> None:
        super().__init__()
...
class WanShiftWindow2DInferProcessor:
    ...
    def __init__(self, window_hw: tuple[int, int] = (16, 16), shift_every_other_layer: bool = True) -> None:
        wh, ww = window_hw
    ...
    def __call__(
        ...
        meta = _WindowRuntimeMetaCache.get(
            T, H, W, wh, ww, do_shift=do_shift, prefer_front=prefer_front, device=hidden_states.device
        )
...
class WanImageEmbedding(nn.Module):
    def __init__(self, in_features: int, out_features: int, pos_embed_seq_len: int | None = None) -> None:
        super().__init__()
        self.norm1 = FP32LayerNorm(in_features)
        self.ff = FeedForward(in_features, out_features, mult=1, activation_fn="gelu")
        self.norm2 = FP32LayerNorm(out_features)
        self.pos_embed = (
            nn.Parameter(torch.zeros(1, pos_embed_seq_len, in_features)) if pos_embed_seq_len is not None else None
        )
...
class WanTimeTextImageEmbedding(nn.Module):
    def __init__(
        ...
        self.image_embedder = (
            WanImageEmbedding(image_embed_dim, dim, pos_embed_seq_len=pos_embed_seq_len)
    ...
    def __init__(
        ...
        for dim in [t_dim, h_dim, w_dim]:
            fc, fs = get_1d_rotary_pos_embed(
                dim, max_seq_len, theta, use_real=True, repeat_interleave_real=True, freqs_dtype=freqs_dtype
            )
            freqs_cos.append(fc)
            freqs_sin.append(fs)
...
def enable_shifted_window_self_attention(model: nn.Module, window_hw: tuple[int, int] = (16, 16)) -> None:
...
def compile_transformer_blocks(model: nn.Module, mode: str = "default") -> None:
    if not hasattr(torch, "compile"):
        logger.warning("torch.compile not available (requires PyTorch 2.0+). Skipping.")
        return
    if mode == "reduce-overhead":
        logger.warning(
            "compile_mode='reduce-overhead' is incompatible with the in-place residuals; falling back to 'default'."
        )
        mode = "default"
    for i, blk in enumerate(getattr(model, "blocks", [])):
...
class SwiftVRWanTransformer3DModel(nn.Module):
    ...
    def __init__(
        ...
        self_attn_window_hw: tuple[int, int] = (16, 16),
    ...
    def state_dict_converter(cls, config: dict | None = None) -> SwiftVRTransformerStateDictConverter:
        return SwiftVRTransformerStateDictConverter(config)
    ...
    def prepare_for_inference(
        ...
        self.set_attention_config(attention_config or AttentionConfig.dense_attention(AttnImplType.TORCH_SDPA))
    ...
    def forward(
        ...
        _WindowRuntimeMetaCache.get(
            ppf, pph, ppw, min(cfg_wh, pph), min(cfg_ww, ppw), do_shift=False, prefer_front=True, device=dev
        )
        _WindowRuntimeMetaCache.get(
...
class SwiftVRTransformerStateDictConverter:
    """Keep the released Diffusers checkpoint keys unchanged."""

    def __init__(self, config: dict | None = None) -> None:
        self.config = dict(config or {})

    def from_diffusers(self, state_dict: dict[str, torch.Tensor]) -> tuple[dict[str, torch.Tensor], dict[str, object]]:
        config = {key: value for key, value in self.config.items() if not key.startswith("_")}
        return state_dict, config

    def from_official(self, state_dict: dict[str, torch.Tensor]) -> tuple[dict[str, torch.Tensor], dict[str, object]]:
