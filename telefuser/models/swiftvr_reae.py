...
def conv(n_in: int, n_out: int, **kwargs: object) -> nn.Conv2d:
...
class ReAE(nn.Module):
    def __init__(
        ...
        if checkpoint_path is not None:
            raise ValueError("Load ReAE checkpoints through ModuleManager")

    @classmethod
    def state_dict_converter(cls) -> SwiftVRReAEStateDictConverter:
        return SwiftVRReAEStateDictConverter()


class SwiftVRReAEStateDictConverter:
    """Keep the released ReAE checkpoint keys unchanged."""

    @staticmethod
    def from_official(state_dict: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        return state_dict

    @staticmethod
    def from_diffusers(state_dict: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        return state_dict
