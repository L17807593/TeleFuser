...
class MiniMaxH3MaterialPlanItem:
    condition_index: int
    role: str
    condition_type: str
    uri: str
...
def _parse_aspect_ratio(value: str) -> tuple[int, int]:
    ...
    try:
        ...
        raise ValueError(f"target.aspect_ratio must be integer 'W:H', got {value!r}") from exc
    if w <= 0 or h <= 0:
        raise ValueError(f"target.aspect_ratio components must be positive, got {value!r}")
    return w, h
...
def _validate_base_short_edge(value: Any) -> int:
    ...
    if short_edge != MINIMAX_H3_BASE_SHORT_EDGE or value != short_edge:
        raise ValueError(f"target.short_edge must be 768 for MiniMax H3 shape policy v2, got {value!r}")
    return short_edge
...
def minimax_h3_resolve_spatial_shape(
    *,
    ...
    try:
        ...
        raise ValueError("shape width and height must be positive finite numbers") from exc
    if (
...
def _resolve_shape(
    ...
    *,
    ...
    frame_count = minimax_h3_align_frame_count(int(round(float(target["duration_seconds"]) * fps)))
