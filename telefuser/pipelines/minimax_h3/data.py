...
MINIMAX_H3_REF2VA_MAX_FILES = 12
MINIMAX_H3_REF2VA_MAX_IMAGES = 9
MINIMAX_H3_REF2VA_MAX_VIDEOS = 3
MINIMAX_H3_REF2VA_MAX_AUDIO = 3
MINIMAX_H3_REF2VA_MIN_CLIP_SECONDS = 2.0
MINIMAX_H3_REF2VA_MAX_CLIP_SECONDS = 15.0
MINIMAX_H3_REF2VA_MAX_TOTAL_SECONDS = 15.0
_ALLOWED_CONDITION_KEYS = frozenset({"type", "uri", "role", "frame_index", "start_time_seconds"})
...
def _validate_target(target: Any, *, profile: MiniMaxH3TaskProfile) -> dict[str, Any]:
    ...
    if short_edge != 768:
        raise ValueError(f"{path}.short_edge must be 768 for minimax_h3, got {short_edge}")
    ...
    if profile.aspect_ratio_forced_auto and aspect_ratio != "auto":
        raise ValueError(f'{path}.aspect_ratio must be "auto" for task {profile.task!r}, got {aspect_ratio!r}')
    ...
    if has_duration:
        ...
        if not (MINIMAX_H3_MIN_DURATION_SECONDS <= float(duration) <= MINIMAX_H3_MAX_DURATION_SECONDS):
            raise ValueError(
...
def _validate_conditions(
    ...
    *,
    ...
    if not profile.conditions_required:
        if len(conditions) > 0:
            raise ValueError(f"{path} must be empty for task {profile.task!r} (got {len(conditions)} entries)")
        return []
    if len(conditions) == 0:
        raise ValueError(f"{path} requires at least one entry for task {profile.task!r}")
    if profile.min_condition_count is not None and len(conditions) < profile.min_condition_count:
        raise ValueError(
            f"{path} requires at least {profile.min_condition_count} entries "
            f"for task {profile.task!r}, got {len(conditions)}"
        )
    if profile.max_condition_count is not None and len(conditions) > profile.max_condition_count:
        raise ValueError(
            f"{path} allows at most {profile.max_condition_count} entries "
            f"for task {profile.task!r}, got {len(conditions)}"
        )

    aligned_frame_count = minimax_h3_align_frame_count(frame_count) if frame_count is not None else None
    ...
    for index, cond in enumerate(conditions):
        ...
        if role not in (
            ...
            raise ValueError(f"{cpath}.role must be keyframe or reference, got {role!r}")
        ...
        if rule.requires_frame_index:
            ...
            if aligned_frame_count is None:
                raise ValueError(f"{cpath}.frame_index requires a resolved target duration")
            if frame_index == -1:
        ...
        if start_time_seconds is not None:
            if cond_type not in {"video", "video_audio"}:
                raise ValueError(f"{cpath}.start_time_seconds is only allowed for video or video_audio references")
...
def _validate_ref2va_conditions(conditions: Sequence[Mapping[str, Any]]) -> None:
    if len(conditions) > MINIMAX_H3_REF2VA_MAX_FILES:
        raise ValueError(
            f"conditions allows at most {MINIMAX_H3_REF2VA_MAX_FILES} files for task 'ref2va', got {len(conditions)}"
        )
    image_count = sum(condition["type"] == "image" for condition in conditions)
    video_count = sum(condition["type"] in {"video", "video_audio"} for condition in conditions)
    audio_count = sum(condition["type"] in {"audio", "video_audio"} for condition in conditions)
    if image_count > MINIMAX_H3_REF2VA_MAX_IMAGES:
        raise ValueError(
            f"conditions allows at most {MINIMAX_H3_REF2VA_MAX_IMAGES} image references, got {image_count}"
        )
    if video_count > MINIMAX_H3_REF2VA_MAX_VIDEOS:
        raise ValueError(
            f"conditions allows at most {MINIMAX_H3_REF2VA_MAX_VIDEOS} video references, got {video_count}"
        )
    if audio_count > MINIMAX_H3_REF2VA_MAX_AUDIO:
        raise ValueError(f"conditions allows at most {MINIMAX_H3_REF2VA_MAX_AUDIO} audio references, got {audio_count}")
    has_visual = any(condition["type"] in {"image", "video", "video_audio"} for condition in conditions)
    if audio_count and not has_visual:
        raise ValueError("conditions audio references require at least one image or video reference")
...
def minimax_h3_validate_reference_media_facts(
    ...
    *,
    ...
    if audio_total > MINIMAX_H3_REF2VA_MAX_TOTAL_SECONDS:
        raise ValueError(
...
def minimax_h3_validate_canonical_request(
    ...
    **_extra_kwargs: Any,
    ...
    if normalized_target.get("duration_seconds") is not None:
        requested_frame_count = int(round(float(normalized_target["duration_seconds"]) * MINIMAX_H3_SUPPORTED_FPS))
    ...
    if normalized_target.get("duration_seconds") is None:
        ...
        duration_sources = [cond for cond in normalized_conditions if cond["type"] in ("audio", "video", "video_audio")]
        ...
        if len(duration_sources) > 1:
            raise ValueError("target.duration_seconds is required when multiple audio-bearing references are provided")
    ...
    normalized_audio_flow_shift = _optional_positive_finite_float(audio_flow_shift, "audio_flow_shift")
    ...
    if seed is not None:
        ...
        if normalized_seed > MINIMAX_H3_MAX_SIGNED_SEED:
            raise ValueError(f"seed must not exceed the signed int64 maximum, got {normalized_seed}")
