...
def _rope_with_offset(rope, ppf, pph, ppw, t_off=0, h_off=0, w_off=0):
    ...
    return (
        torch.cat([cf, ch, cw], -1).reshape(1, ppf * pph * ppw, 1, -1),
...
class StreamingDiT:
    ...
    def __init__(self, transformer: SwiftVRWanTransformer3DModel, overlap: int = 0) -> None:
    ...
    @torch.inference_mode()
    def denoise(self, lq: torch.Tensor, prompt_emb: torch.Tensor) -> torch.Tensor:
    ...
    def denoise_last_chunk(
        self,
        z_new_ntchw: torch.Tensor,
        spec: ChunkSpec,
        prompt_emb: torch.Tensor,
        prev_dit_out_cpu: torch.Tensor | None,
        n_lat: int,
        device: torch.device,
        dtype: torch.dtype,
    ) -> torch.Tensor:
