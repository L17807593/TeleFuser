...
from telefuser.models.swiftvr_reae import MemBlock, ReAE, TGrow, TPool

from .chunk import ChunkSpec, ChunkType


def apply_parallel_with_boundary(
...
class StreamingTAE:
    def __init__(self, model: ReAE) -> None:
        self.model = model
        self._enc_st = None
        self._dec_st = None
        self._enc_left = None
    ...
    @torch.no_grad()
    def decode_chunk_fixed(self, z: torch.Tensor, spec: ChunkSpec) -> torch.Tensor | None:
    ...
    @torch.no_grad()
    def encode_chunk(self, x: torch.Tensor) -> torch.Tensor | None:
        ps = self.model.patch_size
        if ps > 1:
            N, T, C, H, W = x.shape
            x = F.pixel_unshuffle(x.reshape(N * T, C, H, W), ps)
    ...
    @torch.no_grad()
    def decode_chunk(self, z: torch.Tensor) -> torch.Tensor | None:
        x, self._dec_st = apply_parallel_with_boundary(self.model.decoder, z, self._dec_st)
        if x is None:
            return None
        x = self._postprocess(x)
